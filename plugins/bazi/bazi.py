...
class BaziPlugin(Plugin):
    ...
    def 命盘分析(self, parent, 本命盘四个天干名字列表, 本命盘四个地支名字列表, 四个天干十神列表,性别):     #对命盘进行详细分析
        ...
        def 整理天干地支零碎的信息(self):
            ...
            for dayun in yun.getDaYun()[1:]:   
                ...
                empty = chr(12288)
                if zhi_ in 地支空亡字典[self.zhus[2]]:  
                    empty = '地支空亡'        
                
                #《打印大运》
                jia = ""
                if gan_ in self.本命盘四个天干名字列表:
                    for i in range(4):
                        if gan_ == self.本命盘四个天干名字列表[i]:
                            if abs(self.地支列表.index(zhi_) - self.地支列表.index(self.本命盘四个地支名字列表[i])) == 2:
                                jia = jia + "  --夹：" +  self.地支列表[( self.地支列表.index(zhi_) + self.地支列表.index(self.本命盘四个地支名字列表[i]) )//2]
                            if abs(self.地支列表.index(zhi_) - self.地支列表.index(self.本命盘四个地支名字列表[i]) ) == 10:
                                jia = jia + "  --夹：" +  self.地支列表[(self.地支列表.index(zhi_) + self.地支列表.index(self.本命盘四个地支名字列表[i]))%12]
                ...
                out = "{age:<3d}岁    {year:<7s}{ganzhi:<5s}   {shi_shen}    {empty} {fu}     {gan}:{十神关系}{yinyang}      {zhi}:{地支_十神关系}{yinyang}          {zhi_info}".format(         
                    ...
                    十神关系=十神关系表[me][gan2_], yinyang=self.计算阴阳属性(zhi2_), 
                    zhi=zhi2_, 地支_十神关系=十神关系表[me][zhi2_], zhi_info=zhi__)   #zhi__ 是用于存储大运地支与本命盘地支刑冲合害关系的关系
                    #zhi_canggan=地支藏干字典_,
                print(out)

                gan_index = self.天干列表.index(gan_)
                zhi_index = self.地支列表.index(zhi_)


                zhis2 = list(self.本命盘四个地支名字列表) + [zhi_]      #计算每个大运中的流年
                gans2 = list(self.本命盘四个天干名字列表) + [gan_]
                for liunian in dayun.getLiuNian():
                    gan2_ = liunian.getGanZhi()[0]     #获取流年的天干（gan2_）和地支（zhi2_）
                    zhi2_ = liunian.getGanZhi()[1]
                    fu2 = '*' if (gan2_, zhi2_) in self.zhus else " "  #检查这个干支组合是否与本命盘的四柱相同。如果相同，就标记一个星号，表示特殊的本命年
                    ...
                    #《打印流年》
                    empty = chr(12288)
                    if zhi2_ in 地支空亡字典[self.zhus[2]]:
                        empty = '地支空亡'       
                    age = liunian.getAge()
    ...
    # 7. 日支🌳、月干🌕、时干⏰ 定用神/忌神
        ...
        def 分析天干十神的右环境吉凶():
            ...
            for 天干 in ['年干', '月干', '时干']:
                ...
                是正十神 = 十神 in ['正官', '正印', '正财', '伤官', '劫财']
                if (self.性别变量 == '男' and 是正十神) or (self.性别变量 == '女' and not 是正十神):
                    环境类型字典[天干] = "内心环境"
                else:
                    环境类型字典[天干] = "社会环境"
            ...
            if self.本命盘四个天干名字列表 and self.本命盘四个地支名字列表:
                ...
                try:
                    self.命盘分析(
                        ...
                        四个天干十神列表=[十神关系表[self.本命盘四个天干名字列表[2]][干] for 干 in self.本命盘四个天干名字列表],
                        性别=self.性别
                    )
